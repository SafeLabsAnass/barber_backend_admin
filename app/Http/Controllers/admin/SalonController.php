...
class SalonController extends CommonController
{
    ...
    public function index()
    {
        ...
        $workinghours = WorkingHours::where(
            "salon_id",
            $salon->salon_id
        )->get();
        return view(
    ...
    public function store(Request $request)
    {
        ...
        if ($request->hasFile("image")) {
            $image = (new Helper())->imageUpload(
                $request->image,
                "salon logos"
            );
            $salon->image = $image;
        } else {
            $salon->image = "default.jpg";
        }

        if ($request->hasFile("logo")) {
            $image = (new Helper())->imageUpload($request->logo, "salon logos");
            $salon->logo = $image;
        } else {
            $salon->logo = "logo.png";
        }
        ...
        $salon->start_time = Carbon::parse($request->start_time)->format(
            "h:i A"
        );
    ...
    public function update(Request $request, $id)
    {
        $request->validate([
            ...
            "website" => "",
            "phone" =>
                "bail|required|numeric|unique:salon,phone," . $id . ",salon_id",
            "address" => "bail|required",
            "city" => "bail|required",
            "state" => "bail|required",
            "country" => "bail|required",
        ...
        if ($request->hasFile("image")) {
            if (
                \File::exists(
                    public_path("/storage/images/salon logos/" . $salon->image)
                )
            ) {
                \File::delete(
                    public_path("/storage/images/salon logos/" . $salon->image)
        ...
        if ($request->hasFile("logo")) {
            if (
                \File::exists(
                    public_path("/storage/images/salon logos/" . $salon->logo)
                )
            ) {
                \File::delete(
                    public_path("/storage/images/salon logos/" . $salon->logo)
        ...
        foreach ($workingHours as $workingHour) {
            ...
            $workingHour->status = isset(
                $data["status" . $workingHour->day_index]
            )
                ? 1
                : 0;
    ...
    public function salonDayOff(Request $request)
    {
        ...
        $salon->$salon_day = json_encode(["open" => null, "close" => null]);
        $salon->save();
    }
}
