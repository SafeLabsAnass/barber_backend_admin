        ...
        if (isset($request->filter_date)) {
            if ($request->filter_date != null) {
                ...
                foreach ($users as $user) {
                    $user->appointment = Booking::where([
                        ["user_id", $user->id],
                ...
                $users = $users
                    ->sortByDesc("appointment")
                    ->values()
                    ->all();
                return view(
                    "admin.report.userReport",
                    compact("users", "pass")
                );
            } else {
                return redirect("/admin/report/user")->withErrors([
    ...
    public function revenue(Request $request)
    {
        if (isset($request->filter_date)) {
            if ($request->filter_date != null) {
                ...
                return view(
                    "admin.report.revenueReport",
                    compact("bookings", "setting", "pass")
                );
            } else {
                return redirect("/admin/report/revenue")->withErrors([
                    "Select Date In Range",
                ]);
            }
        } else {
