...
class CalendarController extends CommonController
{
    ...
    public function index()
    {
        $salon_id = Salon::where("owner_id", Auth()->user()->id)->first()
            ->salon_id;
        $appointments = Booking::with("user")
        ...
        foreach ($appointments as $appointment) {
            ...
            } elseif ($appointment->booking_status == "Approved") {
                $appointment["bgColor"] = "rgba(136, 230, 247, .5)";
                $appointment["textColor"] = "#05879e";
            } elseif ($appointment->booking_status == "Completed") {
        ...
        return view(
            "admin.pages.calendar",
            compact("appointments", "users", "services")
        );
    }
}
