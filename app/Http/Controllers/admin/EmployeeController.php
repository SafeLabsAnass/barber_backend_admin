...
class EmployeeController extends CommonController
{
    ...
    public function store(Request $request)
    {
        ...
        if ($request->hasFile("image")) {
            $emp->image = (new Helper())->imageUpload(
                $request->image,
                "employee"
            );
        }
    ...
    public function show($id)
    {
        ...
        return view(
            "admin.employee.show",
            compact("emp", "appointment", "client", "symbol", "workinghours")
        );
    }
    ...
    public function update(Request $request, $id)
    {
        $request->validate([
            "name" => "bail|required",
            "email" =>
                "bail|required|email|unique:employee,email," . $id . ",emp_id",
            "services" => "bail|required",
            "phone" =>
                "bail|required|numeric|unique:employee,phone," .
                $id .
                ",emp_id",
        ...
        if ($request->hasFile("image")) {
            if ($emp->image != "noimage.jpg") {
                if (
                    \File::exists(
                        public_path("/storage/images/employee/" . $emp->image)
                    )
                ) {
                    \File::delete(
                        public_path("/storage/images/employee/" . $emp->image)
