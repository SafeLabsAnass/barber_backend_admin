...
class OfferController extends CommonController
{
    ...
    public function store(Request $request)
    {
        ...
        if ($request->hasFile("image")) {
            $offer->image = (new Helper())->imageUpload(
                $request->image,
                "offer"
            );
        }
        ...
        return response()->json(
            ["success" => true, "data" => $offer, "msg" => "Offer create"],
            200
        );
    }
    ...
    public function show($id)
    {
        ...
        return response()->json(
            ["success" => true, "data" => $data, "msg" => "Offer show"],
            200
        );
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($request->hasFile("image")) {
            if (
                \File::exists(
                    public_path("/storage/images/offer/" . $offer->image)
                )
            ) {
                \File::delete(
                    public_path("/storage/images/offer/" . $offer->image)
        ...
        $offer->save();
        return response()->json(
            ["success" => true, "data" => $offer, "msg" => "Offer edit"],
            200
        );
    }
