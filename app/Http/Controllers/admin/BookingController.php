...
class BookingController extends CommonController
{
    ...
    public function index()
    {
        ...
        return view(
            "admin.pages.booking",
            compact("bookings", "symbol", "users", "services", "emps")
        );
    }
    ...
    {
        $salon_id = Salon::where("owner_id", Auth()->user()->id)->first()
            ->salon_id;
        ...
        return view(
            ...
            compact("services", "users", "emps")
        );
    }

    public function paymentcount(Request $request)
    {
        $data["price"] = Service::whereIn("service_id", $request->ser_id)->sum(
            "price"
        );
        $data["time"] = Service::whereIn("service_id", $request->ser_id)->sum(
            "time"
        );

        return response()->json(
            ["success" => true, "data" => $data, "msg" => "Single Service"],
            200
        );
    }
    ...
    public function store(Request $request)
    {
        ...
        $duration = Service::whereIn("service_id", $request->service_id)->sum(
            "time"
        );
        $start_time = new Carbon(
            $request["date"] . " " . $request["start_time"]
    ...
    public function show($id)
    {
        ...
        return response()->json(
            ["success" => true, "data" => $data, "msg" => "Appointment show"],
            200
        );
    }
